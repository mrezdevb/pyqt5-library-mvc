...
class LoanService:
    ...
    def loan_book(self, member_id: str, isbn: str) -> Tuple[bool, str]:
        try:
            ...
            active_loans_book: int = self.loan_repo.count_active_loans_by_member(
                member_id
            ).count()
            self._log(
                "debug",
                "ACTIVE_LOANS_COUNT",
                msg=f"""Member {member_id} currently has
                  {active_loans_book} active loans.""",
                member_id=member_id,
                active_loans=active_loans_book,
            )
    ...
    def return_book(self, member_id: str, isbn: str) -> Tuple[bool, str]:
        try:
            ...
            loan: Optional[Loan] = self.loan_repo.get_loans_by_member(
                member_id, isbn
            ).first()
            ...
            if not loan:
                ...
                self._log(
                    "warning", "LOAN_NOT_FOUND", msg=msg, member_id=member_id, isbn=isbn
                )
                return False, msg
            ...
            self._log(
                "exception",
                "RETURN_BOOK_ERROR",
                msg=f"""Error while returning book
                  {isbn} for member {member_id}: {str(e)}""",
                member_id=member_id,
                isbn=isbn,
                error=str(e),
            )
