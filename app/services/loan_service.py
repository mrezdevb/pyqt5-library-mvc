...
class LoanService:
	...
	def loan_book(self, member_id: str, isbn: str) -> Tuple[bool, str]:
		try:
			...
			if not member:
				msg: str = f'Member ID {member_id} not found'
			...
			active_loans_book: int = self.loan_repo.count_active_loans_by_member(member_id).count()
			...
			if book.is_borrowed:
				msg: str = f'Book with ISBN {isbn} is already borrowed.'
				self._log("warning", "BOOK_ALREADY_BORROWED", msg=msg, isbn=isbn)
				return False, msg

			if active_loans_book >= max_borrow_limit:
				msg: str = f'This member already has {max_borrow_limit} borrowed books.'
			...
			msg: str= f'Book "{book.title}" loaned to {member.name}.'
	...
	def return_book(self, member_id: str, isbn: str) -> Tuple[bool, str]:
		try:
			...
			if not book:
				msg: str = f'Book ISBN {isbn} not found'
			...
			loan: Optional[Loan] = self.loan_repo.get_loans_by_member(member_id, isbn).first()
	
			if not loan:
				msg: str = f'Book {isbn} was not loaned by member {member_id}'
			...
			msg: str = f'Book "{book.title}" returned by member {member_id}'
