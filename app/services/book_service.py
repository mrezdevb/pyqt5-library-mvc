...
class BookService:
    ...
    def remove_book(self, isbn: str) -> Tuple[bool, str]:
        try:
            ...
            if not book:
                ...
                self._log("warning", "BOOK_REMOVE_NOT_FOUND", msg=msg, isbn=isbn)
                return False, msg
            ...
            self._log(
                "info", "BOOK_REMOVE_SUCCESS", msg=msg, title=book.title, isbn=isbn
            )
    ...
    def show_books(self, filter_option: Optional[str]) -> List[Book]:
        try:
            self._log(
                "info",
                "BOOK_FETCH_START",
            ...
            if filter_option == "All Books":
                self._log(
                    "debug", "BOOK_FETCH_ALL", msg="Fetching all books from database."
                )
                books: List[Book] = self.book_repo.query_all().all()
            else:
                self._log(
                    "debug",
                    "BOOK_FETCH_AVAILABLE",
            ...
            self._log(
                "exception",
                "BOOK_FETCH_ERROR",
                msg=f"""Error while fetching books with filter
                 "{filter_option}": {str(e)}""",
                filter_option=filter_option,
                error=str(e),
            )
    ...
    def search_books(
        self, keyword: Optional[str], filter_option: str
    ) -> Tuple[List[Book], str]:
        try:
            self._log(
                "info",
                "BOOK_SEARCH_START",
                msg=f'''Searching books with keyword "{keyword}"
                  and filter "{filter_option}"''',
                keyword=keyword,
                filter_option=filter_option,
            )
            ...
            self._log(
                "debug",
                "BOOK_SEARCH_QUERY_EXECUTED",
                msg=f"""Query executed, {len(all_results)}
                  result(s) found.""",
                keyword=keyword,
                filter_option=filter_option,
            ...
            if filter_option != "All Books":
                ...
                self._log(
                    "debug",
                    "BOOK_SEARCH_FILTER_APPLIED",
                    msg="Applying availability filter (only not borrowed).",
                    keyword=keyword,
                    filter_option=filter_option,
                )
                available_books: Optional[List[Book]] = (
                    self.book_repo.query_available_books(keyword).all()
                )
                ...
                self._log(
                    "info",
                    "BOOK_SEARCH_AVAILABLE_SUCCESS",
                    msg=f"""{len(available_books)} available book(s)
            ...
            self._log(
                "info",
                "BOOK_SEARCH_SUCCESS",
                msg=f"""{len(all_results)} book(s) found for keyword "{keyword}"
                  with filter "All Books".""",
                keyword=keyword,
                filter_option=filter_option,
                results_count=len(all_results),
            )
            ...
            self._log(
                "exception",
                "BOOK_SEARCH_ERROR",
                msg=f"""Error while searching books with keyword "{keyword}"
                  and filter "{filter_option}": {str(e)}""",
                keyword=keyword,
                filter_option=filter_option,
                error=str(e),
            )
