...
class MemberService:
    ...
    def add_member(self, name: str, member_id: str) -> Tuple[bool, str]:
        try:
            ...
            existing_member: Optional[Member] = self.member_repo.query_by_member_id(
                member_id
            ).first()
            if existing_member:
                msg: str = f"Member with ID {member_id} already exists."
                self._log("warning", "MEMBER_ADD_EXISTS", msg=msg, member_id=member_id)
                return False, msg
            ...
            self._log(
                "info", "MEMBER_ADD_SUCCESS", msg=msg, name=name, member_id=member_id
            )
    ...
    def remove_member(self, member_id: str) -> Tuple[bool, str]:
        try:
            ...
            member: Optional[Member] = self.member_repo.query_by_member_id(
                member_id
            ).first()
            if not member:
                msg: str = f"Member with ID {member_id} not found."
                self._log(
                    "warning", "MEMBER_REMOVE_NOT_FOUND", msg=msg, member_id=member_id
                )
                return False, msg
            ...
            for loan in loans:
                if loan.book:
                    self._log(
                        "info",
                        "BOOK_RELEASE",
                        msg=f"""Releasing book "{loan.book.title}"
                         (ISBN {loan.book.isbn}).""",
        ...
        try:
            ...
            for member in members:
                borrowed_books: List[str] = [
                    ...
                    if loan.book and not loan.returned
                ]
                self._log(
                    "debug",
                    "MEMBER_FORMAT_DETAIL",
                    msg=f"""Member {member.member_id} has {len(borrowed_books)}
                      active borrowed book(s).""",
                    name=member.name,
                ...
                result.append(
                    {
                        ...
                        "member_id": member.member_id,
                        "borrowed_books": (
                            ", ".join(borrowed_books) if borrowed_books else "-"
                        ),
                    }
                )

            self._log(
                "debug", "MEMBER_FORMAT_COMPLETE", msg="Member formatting complete."
            )
    ...
    def show_members(self, raw: bool = False) -> Union[List[Member], List[dict]]:
        try:
            ...
            members: List[Member] = (
                self.member_repo.get_members_with_loans_and_books().all()
            )

            if not members:
                self._log("info", "MEMBER_FETCH_EMPTY", msg="No members available.")
                return []
    ...
    def search_members(
        self, keyword: Optional[str], raw: bool = False
    ) -> Union[List[Member], List[dict]]:
        try:
            self._log(
                "info",
                "MEMBER_SEARCH_START",
            ...
            results: List[Member] = (
                self.member_repo.search_members_with_loans_and_books(keyword).all()
            )
